...
public class Kuznechik {
    private final int BLOCK_SIZE = 16; // длина блока
    ...
    private String s;// = "Hello Alice ! My name is Bob. How are you ?";

    private byte[] blk;// = s.getBytes();// DatatypeConverter.parseHexBinary("8899aabbccddeeff0077665544332211");

    Kuznechik(String key)//, String mes)
    {
    ...
    public String encr(String mes)
    {
        ...
        for(int i = 0;i < numBlocks;i++) {
            ...
            //res1 += new String ((encriptBlok), StandardCharsets.UTF_8);
            //res1 += new String(encriptBlok, StandardCharsets.UTF_8);
            res1 = res1 + DatatypeConverter.printHexBinary(encriptBlok);
        }
        //System.out.println("исходное сообщение " + s);
        //System.out.println("зашифрованное сообщение " + res1);
        return res1;
    }
    ...
    public String decr(String mes)
    {
        ...
        for(int i = 0;i < numBlocks;i++) {
            //blk = DatatypeConverter.printHexBinary(mes.substring(start, end)).getBytes();



            //blk = mes.substring(start, end).getBytes();


            byte[] tmp = GOST_Kuz_Decript(DatatypeConverter.parseHexBinary(mes.substring(start, end)));
            start += 32;
            end += 32;
            //byte[] decriptBlok = GOST_Kuz_Decript(blk);
            //System.out.println(DatatypeConverter.printHexBinary(decriptBlok));
            //res1 += new String (decriptBlok, StandardCharsets.UTF_8);
            res1 = res1 + new String (tmp, StandardCharsets.UTF_8);
        }
        //System.out.println("исходное сообщение " + s);
        //System.out.println("расшифрованное сообщение " + res1);
        return res1;
    }
}
